...
class ECSVExport_extended
{
    ...
    protected function _writeHeaders($row) {
        ...
        if ($row instanceof CActiveRecord || $row instanceof Sample) {
        ...
        foreach ($row as $key => $value) {
            if ($key == 'notes') {
                ...
                foreach ($value as $note) {
                    $notesCSVValue.=$note->key . ' : ' . $note->value . '; ';
                }

                $row[$key] = $notesCSVValue;
            }
        }
...
//            $csv_value += $key + ", ";
//        }
//        $csv_value = rtrim($csv_value, ",");

        if ($this->stripNewLines) {
            array_walk($row, array('ECSVExport_extended', 'lambdaFail'));
        }
