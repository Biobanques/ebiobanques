...
class EMongoCriteria extends CComponent
{
    ...
    protected $orGroups = array();

    public function addOrGroup($groupName) {
        if (!isset($this->_conditions['$and']))
            $this->_conditions['$and'] = array();
        $this->_conditions['$and'][] = array('$or' => $this->orGroups[$groupName]);
    }

    public function createOrGroup($name) {
        $this->orGroups[$name] = array();
    }

    public function addCondToOrGroup($groupName, $condition) {
        $this->orGroups[$groupName][] = $condition;
    }
